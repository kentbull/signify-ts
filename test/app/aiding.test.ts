...
describe('Aiding', () => {
    ...
    it('Encodes identifier names when adding end roles', async () => {
        const aid1 = await createMockIdentifierState('aid 1', bran, {});
        client.fetch.mockResolvedValueOnce(Response.json(aid1));
        client.fetch.mockResolvedValueOnce(Response.json({}));

        await client.identifiers().addEndRole('aid 1', 'agent');
        const lastCall = client.getLastMockRequest();
        assert.equal(lastCall.path, '/identifiers/aid%201/endroles');
    });
