...
// @ts-ignore
import signify, {b, Seqner, Counter, Credentials, CtrDex, Serder, SignifyClient} from "signify-ts"
...
async function issueCredentialMultisig() {
    ...
    await createMultisigRegistry({client1, client2, client3, aid1, aid2, aid3})


    await issueCredentialWithMultisigAID({client1, client2, client3, aid1, aid2, aid3})
...
async function issueCredentialWithMultisigAID({client1, client2, client3, aid1, aid2, aid3}) {
    ...
    let [cred, credIss, credIxn, redSigs, credRes] = await (client1 as SignifyClient).credentials().create(
        member1AIDName, registries[0]["regk"], vcdata, schemaSAID, aid2.prefix, undefined, undefined, false)
